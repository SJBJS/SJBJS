...
static void Level1()
{
	...
	(new Sharpbox())->SetPosition(rand() % 1400 + 800, -250);
	(new Sharpbox())->SetPosition(rand() % 1400 + 800, -250);
	(new Sharpbox())->SetPosition(rand() % 1400 + 800, -250);
	(new Sharpbox())->SetPosition(rand() % 1400 + 800, -250);
	(new Sharpbox())->SetPosition(rand() % 1400 + 800, -250);
	...
	Ground *m_ground[4];
	LeftGround *m_leftground[4];
	RightGround *m_rightground[4];
	...
	for (int i = 0; i < 4; i++) {
		...
		m_ground[i]->SetPosition((i * 100) * 5, -300);
		m_leftground[i]->SetPosition((i * 100) * 5, -300);
		m_rightground[i]->SetPosition((i * 100) * 5, -300);

		m_ground[i]->SetLeftRight(m_leftground[i], m_rightground[i]);
	}

	StageFlag *m_flag = new StageFlag();
	m_flag->SetPosition(10000, 0);
