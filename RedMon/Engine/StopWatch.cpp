...
{
	deltaTime = checkAndComputeDT() / 1000000.0;
	currentTime = getTotalElapsedTime() / 1000000.0;
}

double CStopWatch::GetDeltaTime()
{
	if (!myWatch)
		return -1;
	return myWatch->deltaTime;
}

double CStopWatch::GetCurrent()
{
	if (!myWatch)
		return -1;
	return myWatch->currentTime;
}
